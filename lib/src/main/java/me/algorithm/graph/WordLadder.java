 ...
 * -> cog", which is 5 words long. Example 2:
 * <p>
 ...
 * wordList[i].length == beginWord.length beginWord, endWord, and wordList[i] consist of lowercase
 * English letters. beginWord != endWord All the words in wordList are unique.
 */
public class WordLadder {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        for (String s : wordList) {
          if (!visited.contains(s) && isNeighbor(curr, s)) {
            queue.offer(s);
            visited.add(s);
          }
  ...
  private boolean isNeighbor(String begin, String end) {
    int diff = 0;
